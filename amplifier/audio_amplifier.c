...
extern int tfa9895_calibration(void);
extern int tfa9895_speakeron(uint32_t);
extern int tfa9895_speakeroff();
...
static int amp_enable_output_devices(struct amplifier_device *device, uint32_t devices, bool enable)
{
    ...
    if ((devices & SMART_PA_DEVICES_MASK) != 0) {
        if (enable) {
            ...
            if ((ret = tfa9895_speakeron(mode)) != 0) {
                ALOGI("tfa9895_speakeron(%d) failed: %d\n", mode, ret);
            }
        } else {
            if ((ret = tfa9895_speakeroff()) != 0) {
                ALOGI("tfa9895_speakeroff failed: %d\n", ret);
            }
        ...
        hw_device_t **device)
{
    ...
    *device = (hw_device_t *) amp_dev;
    ...
    set_clocks_enabled(true);
    if ((ret = tfa9895_calibration()) != 0) {
        ALOGI("tfa9895_calibration failed: %d\n", ret);
    }
    set_clocks_enabled(false);
